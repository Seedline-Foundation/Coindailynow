...
export default function ComparisonPageClient() {
  const sygn = COMPARISON_ITEMS.find(c => c.name === 'Sygn')!;
  const others = COMPARISON_ITEMS.filter(c => c.name !== 'Sygn');
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <section className="bg-gradient-to-r from-orange-500 to-amber-500 rounded-2xl p-8 md:p-12 text-white text-center mb-16">
          <h2 className="text-2xl md:text-3xl font-bold mb-4">Why Sygn Leads for African Crypto News</h2>
          <p className="text-orange-100 max-w-2xl mx-auto mb-6">
            Sygn is the only platform built from the ground up for Africa&apos;s crypto community.
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: {
              '@type': 'ItemList',
              itemListElement: [sygn, ...others].map((p, i) => ({
                '@type': 'ListItem',
                position: i + 1,
                item: {
                  '@type': 'WebSite',
