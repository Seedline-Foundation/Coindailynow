...
- [ ] **Factsheet pages for top 20 entities** — BTC, ETH, BNB, SOL + Binance, Luno, Quidax, YellowCard + MTN, Safaricom, Standard Bank, Naspers + NG, KE, ZA, GH country profiles. Next.js template + seed JSON. Bloomberg-positioning signal #1. Estimate: 8h.
...
- [ ] **WhatsApp-optimized OG social cards** — Code infrastructure done: `openGraph.images` set to 1200x630 in layout.tsx, `DynamicMetaTags.tsx` handles per-article OG. **Remaining:** create actual `/og-image.png` and `/twitter-image.png` design assets (founder task — use Canva or Figma). Estimate: 30min design work.
- [ ] **Seed the regulatory map** with accurate, human-verified data for NG, KE, ZA, GH. Currently a static tool — needs real content. Estimate: 4h.
- [ ] **Listmonk setup + daily newsletter template** — self-hosted on Contabo ($0). One daily newsletter auto-assembled from top 5 articles via AI. Retention engine from Day 1. Estimate: 6h.
...
- [ ] **Status page** at status.coindaily.online (use [Upptime](https://upptime.js.org) — free, GitHub-hosted, $0 forever).
