...
export class ComfyUIClient {
  ...
  async waitForCompletion(
    ...
    while (Date.now() - startTime < timeoutMs) {
      try {
        ...
        if (error.message?.includes('execution error')) throw error;
      }
   ...
   */
  async getQueueStatus(): Promise<{ queue_running: number; queue_pending: number }> {
    try {
      const response = await fetch(`${this.baseUrl}/queue`, {
        signal: AbortSignal.timeout(5000),
      });
      if (!response.ok) return { queue_running: 0, queue_pending: 0 };
