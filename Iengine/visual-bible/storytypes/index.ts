...
import { StoryType, WorkflowConfig, WorkflowType } from '../../types';
...
export const storyTypeConfigs: Record<StoryType, StoryTypeConfig> = {
  'breaking-news': {
    type: 'breaking-news',
 ...
 */
export function detectStoryType(
  headline: string,
  category?: string,
  tags?: string[]
): StoryType {
  ...
  const scored: { type: StoryType; score: number }[] = [];

  for (const [, config] of Object.entries(storyTypeConfigs)) {
    if (config.keywords.length === 0) continue;
    let score = 0;
    for (const kw of config.keywords) {
      if (text.includes(kw)) score++;
    }
    if (score > 0) scored.push({ type: config.type, score });
  }
 ...
 */
export function getStoryTypeConfig(storyType: StoryType): StoryTypeConfig {
  return storyTypeConfigs[storyType] || storyTypeConfigs['premium-feature'];
}
