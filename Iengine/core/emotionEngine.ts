...
import { EmotionAnalysis, NarrativeAnalysis, StoryType } from '../types';
...
const EMOTION_MAP: Record<string, {
  primary: string;
  visual_mood: string;
  color_temperature: EmotionAnalysis['color_temperature'];
}> = {
  euphoria: {
...
const STORY_TYPE_EMOTIONS: Record<StoryType, {
...
export class EmotionEngine {
  ...
  private calculateIntensity(narrative: NarrativeAnalysis): number {
    let intensity = 0.5;

    switch (narrative.urgency) {
      case 'critical': intensity = 0.95; break;
  ...
  private deriveSecondaryEmotion(narrative: NarrativeAnalysis): string | undefined {
  ...
  private determineValence(
    ...
    if (['euphoria', 'excitement', 'optimism'].includes(marketEmotion)) return 'positive';
    if (['panic', 'fear'].includes(marketEmotion)) return 'negative';
