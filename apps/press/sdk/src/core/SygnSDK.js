/**
 * SENDPRESS SDK - Sygn PR Distribution Network
...
class SygnSDK {
     ...
     */
    loadWebComponents() {
        if (!customElements.get('sygn-pr-card')) {
            customElements.define('sygn-pr-card', PRCard);
        }
        if (!customElements.get('sygn-pr-full')) {
            customElements.define('sygn-pr-full', PRFull);
        }
        if (!customElements.get('sygn-pr-container')) {
            customElements.define('sygn-pr-container', PRContainer);
        }
    }
    ...
    async publishPR(prId, { positionId }) {
        ...
        const element = position.displayType === 'card' 
            ? document.createElement('sygn-pr-card')
            : document.createElement('sygn-pr-full');
    ...
    getSiteSecret() {
        return localStorage.getItem(`sygn_secret_${this.siteId}`);
    }
    
    storeSiteSecret(secret) {
        localStorage.setItem(`sygn_secret_${this.siteId}`, secret);
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { SygnSDK };
}
if (typeof window !== 'undefined') {
    window.SygnSDK = SygnSDK;
    window.Sygn = {
        init: (config) => new SygnSDK(config.siteId, config)
    };
}

export { SygnSDK };
export default SygnSDK;
