 ...
 * Status pulls live from /api/super-admin/distribution/platforms (for adapters)
...
const CHECKLIST: ChecklistItem[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Optional: upload a custom Sygn anchor image; set DID_PRESENTER_IMAGE_URL',
    ],
  ...
  },
  {
    id: 'pexels',
    group: 'video',
    title: 'Pexels — free stock B-roll fallback',
    why: 'Royalty-free video clips matched to scene visualHints. Used automatically when fal.ai is unavailable, so videos still get B-roll for $0.',
    steps: [
    ...
    cost: 'Free (200 req/hr, 20K/month)',
    externalUrl: 'https://www.pexels.com/api/',
    required: false,
  },
  {
    id: 'music-bed',
    group: 'video',
    title: 'Music bed — branded backing track',
    why: 'Mixed under voiceover during compose. Makes shorts feel produced instead of dry. Either point at a single track or wire Pixabay for category-aware picks.',
    steps: [
      'Pick a royalty-free track (e.g. from Pixabay Music or your own license)',
      'Upload the .mp3/.m4a to your CDN and set BRAND_MUSIC_URL=<url>',
      'Optional: override per category with BRAND_MUSIC_URL_CRYPTO, BRAND_MUSIC_URL_REGULATORY, etc.',
      'Optional: set PIXABAY_KEY to dynamically search Pixabay Music per article mood',
    ],
    envVars: ['BRAND_MUSIC_URL', 'PIXABAY_KEY'],
    cost: 'Free (own track or Pixabay)',
    externalUrl: 'https://pixabay.com/music/',
    required: false,
  },
  ...
  // ─── SOCIAL DISTRIBUTION ─────────────────────────────────────────────────
  {
    ...
    steps: [
      'In Telegram, message @BotFather → /newbot → name it SygnBot',
      'BotFather gives you a token — set TELEGRAM_BOT_TOKEN',
      'Create Sygn channel; add bot as admin with post privileges',
      'In /super-admin/distribution → Targets, add target platform=telegram handle=@sygnafrica',
    ],
  ...
  },
  {
    ...
    steps: [
      'Create or claim Sygn YouTube channel',
  ...
  },
  {
    ...
    steps: [
      'Create Sygn company page on LinkedIn',
  ...
  // ─── CDN / STORAGE ──────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      'Create bucket sygn-media (Public)',
...
export default function SetupChecklistPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/super-admin/distribution/platforms`, {
  ...
  return (
    <div className="p-6 max-w-4xl space-y-4">
      <header>
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Setup checklist</h1>
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
              ...
              <span className="ml-3 text-gray-500 dark:text-gray-400">{counts.totalDone}/{counts.total} total</span>
            </p>
      ...
      {Object.entries(grouped).map(([group, items]) => (
        <section key={group} className="rounded-xl border bg-white dark:bg-gray-800">
        ...
        <div className="flex-1 min-w-0">
          <button type="button" onClick={() => setOpen(o => !o)} className="text-left">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-gray-400 dark:text-gray-500">{item.cost}</span>
            </div>
            <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{item.why}</p>
          </button>

          {open && (
            <div className="mt-3 space-y-2 text-xs">
              <div>
                <p className="mb-1 font-medium text-gray-700 dark:text-gray-300">Steps:</p>
                <ol className="ml-5 list-decimal space-y-1 text-gray-600 dark:text-gray-400">
                  {item.steps.map((s, i) => <li key={i}>{s}</li>)}
                </ol>
              </div>
              {item.envVars.length > 0 && (
                <div>
                  <p className="mb-1 font-medium text-gray-700 dark:text-gray-300">Env vars to set in backend/.env:</p>
                  <ul className="ml-5 list-disc space-y-0.5 text-gray-600 dark:text-gray-400">
                    {item.envVars.map(v => (
                      <li key={v}>
                        <code className="rounded bg-gray-100 px-1 py-0.5 font-mono dark:bg-gray-800">{v}</code>
                        <button
                          type="button"
                          onClick={() => navigator.clipboard.writeText(v)}
                          className="ml-1.5 align-middle text-gray-400 hover:text-gray-700 dark:text-gray-500"
                          title="Copy"
                        >
