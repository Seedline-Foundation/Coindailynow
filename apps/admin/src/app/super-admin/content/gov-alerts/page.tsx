...
export default function GovAlertsPage() {
  ...
  if (error && !alerts.length) {
    return (
      <div className="p-6">
        <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 dark:bg-red-900/20 p-4 text-red-700">
  ...
  return (
    <div className="p-6 space-y-4">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Government wire</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">
            Incoming regulator / central bank / agency announcements detected by the GovMonitor worker.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="inline-flex items-center gap-1 rounded-lg border bg-white dark:bg-gray-800 px-2 py-1 text-xs">
          ...
          <button
            ...
            onClick={refresh}
            disabled={loading}
            className="inline-flex items-center gap-1.5 rounded-lg border bg-white dark:bg-gray-800 px-3 py-1.5 text-xs font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-900/40 disabled:opacity-50"
          >
      ...
      {!loading && filtered.length === 0 && (
        <div className="rounded-xl border bg-gray-50 dark:bg-gray-900/40 p-6 text-center text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">
      ...
      <ul className="divide-y rounded-xl border bg-white dark:bg-gray-800">
        {filtered.map(a => {
          ...
          return (
            <li key={a.url} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-900/40">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">
                  <span className={`rounded px-1.5 py-0.5 font-medium ${REGION_COLORS[a.region] || 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}>
                    {a.region}
                  </span>
                ...
                <a
                  ...
                  className="mt-1 inline-flex items-center gap-1 font-medium text-gray-900 dark:text-gray-100 hover:text-indigo-700"
                >
                  {a.title}
                  <ExternalLink className="h-3 w-3 opacity-50" />
                </a>
                {a.summary && (
                  <p className="mt-1 line-clamp-2 text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">{a.summary}</p>
                )}
              </div>
              <button
                type="button"
                ...
                className={`shrink-0 inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium ${
                  isPromoted
                    ...
                    : 'border bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:bg-indigo-900/20 hover:text-indigo-700'
                } disabled:opacity-50`}
              >
                {isPromoting ? (
