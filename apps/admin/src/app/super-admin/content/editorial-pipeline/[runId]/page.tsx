...
function ApprovalControls({
  ...
  if (!canApprove && isSubmitted) {
    return (
      <span className="inline-flex items-center gap-2 rounded-lg bg-amber-50 px-3 py-2 text-sm font-medium text-amber-800 dark:bg-amber-900/20">
  ...
  const onApprove = useCallback(
    async (opts: { publishMode: 'now' | 'scheduled'; scheduledAt?: string }) => {
      ...
      try {
        ...
        router.push(`/super-admin/content?highlight=${json.articleId}`);
      } catch (e: any) {
        setError(e.message);
        setApproving(false);
      }
  ...
  return (
    <div className="p-6 space-y-4">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">{data.run.topic}</h1>
      ...
      {flash && (
        <div className="rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-800 dark:bg-green-900/20">
          {flash}
        </div>
      )}
