  ...
  return (
    <div className="p-6 space-y-4">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Editorial pipeline</h1>
      ...
      {!loading && runs && runs.length === 0 && (
        <div className="rounded-xl border bg-gray-50 p-6 text-center text-sm text-gray-500 dark:bg-gray-900/40 dark:text-gray-400">
          ...
          {status === 'READY_FOR_REVIEW' && (
            <span className="block mt-1">Trigger one via <Link href="/super-admin/content/ai-generate" className="text-indigo-600 hover:underline">AI Generate</Link> or wait for a promoted gov alert.</span>
          )}
        </div>
      )}
          ...
          <li key={run.id} className="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <Link
                  href={`/super-admin/content/editorial-pipeline/${run.id}`}
                  className="font-medium text-gray-900 hover:text-indigo-700 inline-flex items-center gap-1 dark:text-gray-100"
                >
                  {run.topic}
      ...
      <div className="mt-4 rounded-xl border border-dashed bg-white p-4 text-xs text-gray-500 dark:bg-gray-800 dark:text-gray-400">
        <p className="font-medium text-gray-700 dark:text-gray-300">Pipeline shortcuts</p>
        <div className="mt-2 flex flex-wrap gap-2">
          <Link href="/super-admin/content/ai-generate" className="inline-flex items-center gap-1 rounded border bg-white px-2 py-1 hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700/50">
            <Sparkles className="h-3 w-3" /> Generate new article
          </Link>
          <Link href="/super-admin/content/gov-alerts" className="inline-flex items-center gap-1 rounded border bg-white px-2 py-1 hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700/50">
