    ...
    <section className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          onClick={refresh}
          disabled={loading}
          className="inline-flex items-center gap-1.5 rounded border bg-white px-3 py-1 text-xs hover:bg-gray-50 disabled:opacity-50 dark:bg-gray-800 dark:hover:bg-gray-700/50"
        >
      ...
      <ul className="divide-y rounded-xl border bg-white dark:bg-gray-800">
        {posts?.length === 0 && (
          <li className="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
            No distribution posts yet. Approve a video at <Link href="/super-admin/vengine" className="text-indigo-600 hover:underline">/super-admin/vengine</Link> to kick off the first run.
          </li>
        )}
        {posts?.map(p => (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  <span className="font-mono text-xs uppercase rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">{p.target.platform}</span>
                  <span className="text-gray-700 dark:text-gray-300">{p.target.handle}</span>
                  <span className="text-gray-400 dark:text-gray-500">·</span>
                  <span className="font-medium text-gray-700 dark:text-gray-300">{p.itemType}</span>
                  <span className="font-mono text-xs text-gray-500 dark:text-gray-400">{p.itemId.slice(0, 8)}…</span>
                  {p.externalUrl && (
                ...
                {p.errorMessage && (
                  <div className="mt-1 rounded bg-red-50 px-2 py-1 text-xs text-red-700 dark:bg-red-900/20">{p.errorMessage}</div>
                )}
                {p.metrics && (
              ...
              <button
                type="button"
                onClick={() => republish(p)}
                className="shrink-0 rounded border px-2 py-1 text-xs hover:bg-gray-50 dark:hover:bg-gray-700/50"
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-600 dark:text-gray-400">Configured platform handles. The dispatcher posts to every <strong>enabled</strong> target.</p>
        <div className="flex items-center gap-2">
          <Link
            href="/super-admin/distribution/settings"
            className="rounded-lg border border-indigo-600 px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-50"
          >
            Full settings →
          </Link>
          <button
      ...
      <ul className="divide-y rounded-xl border bg-white dark:bg-gray-800">
        {loading && <li className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">Loading…</li>}
        ...
        {targets.map(t => (
          <li key={t.id} className="flex items-center gap-3 px-4 py-3 text-sm">
            ...
            <span className="font-mono text-xs uppercase rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">{t.platform}</span>
            <span className="flex-1 text-gray-800 dark:text-gray-200">{t.handle}</span>
            <span className="text-xs text-gray-500 dark:text-gray-400">{t.enabled ? 'enabled' : 'disabled'}</span>
            <button
              type="button"
              onClick={() => setEditing(t)}
              className="rounded border px-2 py-1 text-xs hover:bg-gray-50 dark:hover:bg-gray-700/50"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl dark:bg-gray-800" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-4 space-y-3">
              ...
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded border px-2 py-1.5 text-sm"
                  placeholder="@sygnafrica or page id"
                />
              </label>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">
                <input
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => setEditing(null)} className="rounded px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 dark:text-gray-400">Cancel</button>
...
function PlatformsTab({ headers }: { headers: Record<string, string> | null }) {
  ...
  return (
    <section className="space-y-2">
      <p className="text-sm text-gray-600 dark:text-gray-400">Each adapter checks for required env vars on startup. Configured adapters can post to any target on that platform.</p>
      <ul className="divide-y rounded-xl border bg-white dark:bg-gray-800">
        {platforms.map(p => (
          <li key={p.platform} className="flex items-start gap-3 px-4 py-3 text-sm">
            ...
            <div className="flex-1">
              <span className="font-mono text-xs uppercase rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">{p.platform}</span>
              <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">{p.configured ? 'configured' : 'not configured'}</span>
              <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">{hints[p.platform] || 'See adapter file for setup.'}</div>
            </div>
          </li>
        ))}
      </ul>
...
function StatusIcon({ status }: { status: string }) {
  switch (status) {
    ...
    default: return <AlertCircle className="h-4 w-4 text-gray-400 dark:text-gray-500" />;
  }
}
