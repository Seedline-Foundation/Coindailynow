  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <header className="flex items-start justify-between">
        <div>
          <div className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">{data?.indexType || 'Chima Index'}</div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{name} <span className="text-base font-mono text-gray-400 dark:text-gray-500">({symbol})</span></h1>
          <p className="text-sm text-gray-500 max-w-2xl dark:text-gray-400">{data?.description || blurb}</p>
        </div>
        <div className="flex gap-2 shrink-0">
          ...
          {data && (
            <button onClick={recalculate} disabled={recalculating} className="inline-flex items-center gap-2 rounded-lg border bg-white px-3 py-2 text-sm font-medium hover:bg-gray-50 disabled:opacity-50 dark:bg-gray-800 dark:hover:bg-gray-700/50">
      ...
      {loading && !data && (
        <div className="flex items-center gap-2 p-3 text-gray-500 dark:text-gray-400"><Loader2 className="h-4 w-4 animate-spin" /> Loading…</div>
      )}

      {data && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-xl border bg-white p-5 dark:bg-gray-800">
              <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Current value</p>
              <p className="mt-1 text-3xl font-bold text-gray-900 dark:text-gray-100">{fmt(data.currentValue)}</p>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Last calculated: {data.lastCalculated ? new Date(data.lastCalculated).toLocaleString() : '—'}</p>
            </div>
            {([['24h', data.changePercent24h, data.change24h], ['7d', null, data.change7d], ['30d', null, data.change30d]] as const).map(([label, pct, abs]) => (
              <div key={label} className="rounded-xl border bg-white p-5 dark:bg-gray-800">
                <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">{label} change</p>
                ...
                {typeof pct === 'number' && <p className="text-xs text-gray-500 dark:text-gray-400">{fmt(pct)}%</p>}
              </div>
            ))}
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <section className="rounded-xl border bg-white p-5 dark:bg-gray-800">
              <h3 className="text-sm font-semibold text-gray-900 mb-3 dark:text-gray-100">Country components</h3>
              {components.length === 0 ? (
                <p className="text-xs text-gray-500 dark:text-gray-400">No component breakdown yet. Run recalculate to populate.</p>
              ) : (
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {components.map((c: any, i: number) => (
                      <tr key={i}>
                        <td className="py-2 font-medium text-gray-800 dark:text-gray-200">{c.country || c.name || '—'}</td>
                        <td className="py-2 text-right font-mono text-gray-700 dark:text-gray-300">{fmt(Number(c.value) || 0)}</td>
                        <td className="py-2 text-right text-gray-500 dark:text-gray-400">{c.weight ? `${fmt(Number(c.weight) * 100, 1)}%` : '—'}</td>
            ...
            <section className="rounded-xl border bg-white p-5 dark:bg-gray-800">
              <h3 className="text-sm font-semibold text-gray-900 mb-3 dark:text-gray-100">30-day history</h3>
              {history.length === 0 ? (
                <p className="text-xs text-gray-500 dark:text-gray-400">No history yet. Recalculate seeds the next point.</p>
              ) : (
                <div>
                  <Sparkline points={history.map(p => p.value)} ath={data.allTimeHigh} atl={data.allTimeLow} />
                  <table className="mt-3 w-full text-xs">
                    <thead className="text-gray-500 dark:text-gray-400"><tr><th className="text-left py-1">Date</th><th className="text-right">Value</th></tr></thead>
                    <tbody className="divide-y">
                      {history.slice(-10).reverse().map((p, i) => (
                        <tr key={i}>
                          <td className="py-1 text-gray-600 dark:text-gray-400">{new Date(p.timestamp).toLocaleDateString()}</td>
                          <td className="py-1 text-right font-mono text-gray-700 dark:text-gray-300">{fmt(p.value)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {data.methodology && (
            <section className="rounded-xl border bg-white p-5 dark:bg-gray-800">
              <h3 className="text-sm font-semibold text-gray-900 mb-2 dark:text-gray-100">Methodology</h3>
              <p className="text-sm text-gray-700 whitespace-pre-wrap dark:text-gray-300">{data.methodology}</p>
            </section>
          )}
        </>
      )}
