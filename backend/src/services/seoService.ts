...
Generated by Sygn AI on ${new Date().toISOString()}
`;
  }
...
Return only the JSON array.`;
  ...
  private extractTitleFromContent(content: string): string {
    ...
    return sentences[0]?.trim().substring(0, 60) || 'Sygn Article';
  }

  private generateDescription(content: string): string {
    const words = content.split(/\s+/).filter(word => word.length > 3);
    const desc = words.slice(0, 25).join(' ');
    return desc.length > 10 ? desc + '...' : 'Latest cryptocurrency and blockchain news from Sygn.';
  }
  ...
  async generateSiteLLMsTxt(): Promise<string> {
    ...
    let content = `# Sygn - Africa's Premier Cryptocurrency News Platform

## About
Sygn provides comprehensive cryptocurrency and blockchain news coverage with a focus on African markets, memecoins, and emerging trends.
...
This content index is maintained by Sygn's AI systems for improved discoverability by language models and AI assistants.
